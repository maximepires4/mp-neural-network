...
class Convolutional(Layer):
    ...
    def init_weights(self, method: Lit_W, no_bias: bool) -> None:
        ...
        if method == "he":
            std_dev = np.sqrt(2.0 / (input_depth * self.kernel_size * self.kernel_size), dtype=DTYPE)
        elif method == "xavier":
            std_dev = np.sqrt(1.0 / (input_depth * self.kernel_size * self.kernel_size), dtype=DTYPE)
        ...
        self.kernels = np.random.randn(*kernels_shape).astype(DTYPE) * std_dev
        self.kernels_gradient = np.zeros_like(self.kernels, dtype=DTYPE)

        self.no_bias = no_bias

        if not self.no_bias:
            self.biases = np.random.randn(self.output_depth).astype(DTYPE)
    ...
    def backward(self, output_gradient_batch: NDArray) -> NDArray:
        ...
        if not self.no_bias:
            self.biases_gradient = np.sum(dOut_col, axis=0, dtype=DTYPE)  # type: ignore
...
class AveragePooling2D(Layer):
    ...
    def forward(self, input_batch: NDArray, training: bool = True) -> NDArray:
        self.input_shape = input_batch.shape
        self.windows = im2col(input_batch, self.pool_size, self.stride)
        means = np.mean(self.windows, axis=(4, 5), dtype=DTYPE)
    ...
    def backward(self, output_gradient_batch: NDArray) -> NDArray:
        ...
        d_windows = grad_expanded * np.ones_like(self.windows, dtype=DTYPE) / (self.pool_size * self.pool_size)
        d_windows = d_windows.transpose(0, 3, 1, 2, 4, 5)
...
class BatchNormalization2D(Layer):
    ...
    def build(self, input_shape: int | tuple[int, ...]) -> None:
        ...
        self.gamma = np.ones((1, C, 1, 1), dtype=DTYPE)
        self.gamma_gradient = np.zeros_like(self.gamma, dtype=DTYPE)

        self.beta = np.zeros((1, C, 1, 1), dtype=DTYPE)
        self.beta_gradient = np.zeros_like(self.beta, dtype=DTYPE)

        self.cache_m = np.zeros((1, C, 1, 1), dtype=DTYPE)
        self.cache_v = np.ones((1, C, 1, 1), dtype=DTYPE)
    ...
    def forward(self, input_batch: NDArray, training: bool = True) -> NDArray:
        ...
        if training:
            mean = np.mean(self.input, axis=(0, 2, 3), keepdims=True, dtype=DTYPE)  # type: ignore
            var = np.var(self.input, axis=(0, 2, 3), keepdims=True, dtype=DTYPE)
    ...
    def backward(self, output_gradient_batch: NDArray) -> NDArray:
        self.gamma_gradient = np.sum(  # type: ignore
            self.x_norm * output_gradient_batch,
            axis=(0, 2, 3),
            keepdims=True,
            dtype=DTYPE,
        )
        self.beta_gradient = np.sum(output_gradient_batch, axis=(0, 2, 3), keepdims=True, dtype=DTYPE)  # type: ignore
        ...
        grad: NDArray = (
            ...
            * (
                N * dx_norm
                - np.sum(dx_norm, axis=(0, 2, 3), keepdims=True, dtype=DTYPE)
                - self.x_norm * np.sum(dx_norm * self.x_norm, axis=(0, 2, 3), keepdims=True, dtype=DTYPE)
            )
        )
        return grad
