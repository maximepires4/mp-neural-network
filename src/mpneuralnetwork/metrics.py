...
class RMSE(Metric):
    ...
    def from_mse(self, mse: float) -> float:
        return xp.sqrt(mse, dtype=DTYPE)


class MAE(Metric):
    def __call__(self, y_true: ArrayType, y_pred: ArrayType) -> float:
        return xp.mean(xp.sum(xp.abs(y_true - y_pred), axis=1, dtype=DTYPE), dtype=DTYPE)
...
class R2Score(Metric):
    ...
    def __call__(self, y_true: ArrayType, y_pred: ArrayType) -> float:
        ...
        return 1 - var_tp / (var_tm + self.epsilon)
...
class Accuracy(Metric):
    def __call__(self, y_true: ArrayType, y_pred: ArrayType) -> float:
        ...
        return xp.mean(y_true == y_pred, dtype=DTYPE)
...
class TopKAccuracy(Metric):
    ...
    def __call__(self, y_true: ArrayType, y_pred: ArrayType) -> float:
        ...
        return xp.mean(xp.any(top_k_preds == y_true, axis=1), dtype=DTYPE)
